...
| D2 | 四个内置示例角色（翻译官 / 摘要助手 / 代码审查员 / 学术解说员）在本里程碑定义**具体 system prompt**；首次启动时 seed 到角色列表，**之后与用户自建角色同等可编辑、可删除**；提供"恢复默认角色"入口（仅当列表中不存在时可重新 seed） | 评审修订（2026-09-14）：用户明确内置角色不应被特殊保护，应与普通角色一致可修改/删除；原 FR-1.3"不可删除"同步修订 |
...
- 首次启动（`at:roles` 为空）时，将四个角色 seed 到 `at:roles`，之后它们就是普通角色
...
1. **翻译官**（`prompts/roles/translator.md`）：专精翻译，保留原文格式（代码块/表格），术语按上下文领域处理，输出译文后可附简短术语说明
2. **摘要助手**（`prompts/roles/summarizer.md`）：提取核心论点与关键信息，结构化输出（要点/结论/待确认），不添加原文没有的信息
3. **代码审查员**（`prompts/roles/code-reviewer.md`）：从正确性/性能/可读性/安全四维度审查，给出具体修改建议与代码片段，不做泛泛评价
4. **学术解说员**（`prompts/roles/academic-explainer.md`）：将学术内容转化为通俗解释，标注关键概念与假设，指出论证链条中的薄弱环节
