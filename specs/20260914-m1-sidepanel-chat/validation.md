...
| T1.2 API 配置 | L1 + L2 | 字段校验（URL/必填/数值）、思维强度 12 种组合、预填表命中、连接测试 401/404/超时 |
| T1.3 角色 | L1 + L2 | 内置角色完整性、CRUD、删除内置被拦截、复制 |
...
- [ ] 发送一条消息，流式输出首块 < 3 秒可见（本地网络）
- [ ] 输出中包含表格/代码块时渲染正确（Markdown + 语法高亮）
- [ ] 点击"停止生成"立即中断，已输出内容保留
...
- [ ] 四个内置角色（翻译官/摘要助手/代码审查员/学术解说员）可见，system prompt 非空
- [ ] 内置角色不可删除，但可复制为自建角色
- [ ] 新建会话时强制选择角色
