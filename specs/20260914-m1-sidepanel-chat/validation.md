...
- [ ] 选择一个不支持思维链的模型（如普通 gpt-3.5）后，思维强度开关置灰并提示"该模型不支持思维链"（**部分通过 / 偏差**：测试 gpt-3.5-turbo 后开关**自动关闭**、映射下拉隐藏、提示文案出现；但开关未 `disabled`，可手动再开启并把映射选项带回来——与"置灰"的措辞不符，见 §4 偏差清单 D-2）
...
- [x] 首次启动有四个默认角色（翻译官/摘要助手/代码审查员/学术解说员），system prompt 非空（412–556 字符）
...
- [ ] 新建会话时强制选择角色（**部分**：会话列表点"新建会话"会显示"选择角色"列表；但从空对话直接发送会自动取第一个角色，未强制选择——见 §4 偏差清单 D-3）
- [x] 会话可重命名、删除，删除当前会话后自动切换（重命名落库生效；删除提示"确定删除此会话？此操作不可撤销。"，删除后自动切到其他会话）
- [ ] 会话标题自动取首条用户消息前 20 字（**不通过**：实测全部为"新对话"/"新会话"，首条用户消息未用于生成标题——见 §4 待修缺陷 D-1）
