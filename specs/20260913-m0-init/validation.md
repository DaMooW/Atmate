...
- **构建**：`pnpm build` 通过 → `dist/chrome-mv3`（MV3，157.35 kB）。
...
- **dev 冒烟**（plan 1.5）：`pnpm dev` 正常启动，dev 服务器 `http://localhost:3000`，产出 `dist/chrome-mv3-dev`，无报错。
...
2. Chrome → `chrome://extensions` → 开发者模式 → 加载已解压的扩展程序 → 选 `dist/chrome-mv3`（确认空 sidepanel 可打开）
...
热更新（dev 构建 `dist/chrome-mv3-dev`）：
