...
| D22 | **划词去重：mouseup 优先 + sidepanel 相似选区合并**：① content script 监听从 `selectionchange + 200ms 去抖` 改为 `mouseup 后立即触发（延迟 10ms 确保 selection 已更新）` + `selectionchange 500ms 去抖兜底（键盘选择）`，mouseup 后 1 秒内抑制 selectionchange 重复触发；② sidepanel 收到新卡片时，若与最近 5 秒内同一 URL 的卡片文本有包含关系（子串/超串且较短文本≥较长文本的 30%），则更新已有卡片而非新增 | 用户反馈（2026-09-15）：划词慢时会创建很多划词卡片，但其实都是要划一个内容；原方案 selectionchange 去抖无法解决缓慢划词过程中多次停顿导致的多卡片问题 |
| D23 | **上下文数据预采集所有档位**：创建素材卡片时一次性采集所有档位的上下文数据（containing-paragraph / nearby / page），存入 `contextData`。切换档位时直接使用已有数据，无需异步请求。解决原方案切换档位后上下文不生效的问题（原方案只采集当前档位数据，切换后 contextData 中无对应数据） | 用户反馈（2026-09-15）：相邻段落和整页正文并没有生效；根因是切换档位时只更新了 contextScope 字段，没有重新采集对应档位的 contextData |
| D24 | **移除补充说明输入框**：素材卡片中移除"补充说明"输入框及 `userNote` 字段，简化卡片交互。Prompt 组装中移除【补充说明】分段。用户如需补充说明，直接在 Composer 输入框中输入即可（发送时自动与素材组装） | 用户反馈（2026-09-15）：不需要补充说明，把它去掉 |
