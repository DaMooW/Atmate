...
**验收**（全部可复现勾选）：
- [x] 配置一个真实 OpenAI 兼容端点，30 秒内完成保存→测试→开始对话（2026-09-14 实测 DeepSeek 通过：连接测试 200、reasoning_content 方言正常；其余四家按 §5.1 留作后续补验，不阻塞合并）
- [x] 流式输出首块可见；表格/代码块渲染正确；“停止生成”立即生效（2026-09-14 实测）
- [x] 断网/错 Key/错 URL 三种故障各有明确中文提示且不崩（2026-09-14 实测 401 / 404 / 网络错误三态）
- [ ] 关闭再打开浏览器：会话、角色、配置、激活项全部恢复（用户 2026-09-15 决定暂缓该项；间接证据：扩展重载后数据完好、storage 已落盘 leveldb）
- [x] 状态条数字随每轮更新；伪造 contextLimit=100 触发达限禁发（2026-09-14 实测）
- [x] 思维强度四档产生的请求体 diff 人工核对符合映射定义（关→无字段 / 轻→low / 中→medium / 深→high）
- [x] 基础指令开启时请求体末尾含完备性三条款；关闭开关后立即不再追加（tech §8.1）
...
| 2026-09-15 | M2 | 交互重构（D18-D21，用户验收反馈）：① D18 划词卡片默认已采用——创建后 `adopted: true`，显示绿色"已采用"标识，移除"采用"按钮，"丢弃"改为"移除"；② D19 采用≠填入输入框——已采用卡片不填入 Composer，发送时自动组装卡片+用户输入，发送后清除已参与发送的卡片；③ D20 上下文默认档位=包含选中词的整段——新增 `ContextScope='containing-paragraph'`，采集选区最近块级祖先的完整文本（规范化空白），设为默认档位，原默认 `'selection'` 降级为可选；④ D21 Prompt 组装格式更丰富精准鲁棒——重写为结构化分段：【用户需求】+【素材】{【用户选中的原文】+【相关上下文（按档位标注）】+【来源】（标题+URL）+【补充说明】}，空字段省略，多卡片按【素材 1】【素材 2】依次输出，用户输入为空时输出默认提示。涉及文件：core/types.ts、core/defaults.ts、core/selection/context.ts（新增 getContainingParagraph）、core/messages.ts（ContextData 新增 containingParagraph）、entrypoints/content/context/page-content.ts、entrypoints/content/messaging/send.ts、components/chat/materialTypes.ts（新增 adopted 字段）、components/chat/promptBuilder.ts（重写 buildMaterialPrompt + buildFinalUserPrompt）、components/chat/MaterialCard.tsx（重构）、components/chat/MaterialCardList.tsx（重构）、components/chat/ChatView.tsx（handleSend 组装逻辑）。测试 393 例全绿（新增 getContainingParagraph 7 例 + promptBuilder 重写 + material-card 重写 + defaults 更新），构建 608.48 kB（content.js 44.16 kB）。spec：`specs/20260915-m2-selection-integration/` 任务组 8 |
| 2026-09-15 | M2 | 验收修复（D22-D24，用户二次验收反馈）：① D22 划词去重——content script 从 `selectionchange+200ms去抖` 改为 `mouseup 后立即触发（延迟10ms）+ selectionchange 500ms 去抖兜底（键盘选择）`，mouseup 后 1 秒内抑制 selectionchange 重复触发；sidepanel 新增相似选区合并：新卡片与最近 5 秒内同一 URL 的卡片文本有包含关系（子串/超串且较短文本≥较长文本的 30%）则更新已有卡片而非新增。修复缓慢划词创建大量重复卡片的问题。② D23 上下文数据预采集所有档位——`buildContextData` 用 `Promise.all` 并行采集 containing-paragraph/nearby/page 三档数据，切换档位时直接使用已有数据即时生效，修复切换到±相邻段落/整页正文后上下文不显示的问题（根因：原方案只采集当前档位数据，切换后 contextData 中无对应数据）。③ D24 移除补充说明——MaterialCard 移除补充说明输入框 UI，materialTypes 移除 userNote 字段，promptBuilder 移除【补充说明】分段，ChatView 创建卡片时不设置 userNote。涉及文件：entrypoints/content/selection/monitor.ts（重写）、components/chat/ChatView.tsx（去重逻辑）、entrypoints/content/context/page-content.ts（预采集）、entrypoints/content/messaging/send.ts（移除 contextScope 参数）、components/chat/MaterialCard.tsx（移除补充说明）、components/chat/materialTypes.ts（移除 userNote）、components/chat/promptBuilder.ts（移除补充说明）。测试 394 例全绿（content-monitor 重写 11 例 + prompt-builder 移除补充说明测试 + material-card 新增不显示补充说明测试），构建 608.66 kB（content.js 44.40 kB）。spec：`specs/20260915-m2-selection-integration/` 任务组 9 |
