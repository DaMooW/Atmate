...
- [ ] T2.1 content script：selectionchange 去抖；选区有效性判定（可编辑元素/输入框内选区不弹浮层）
...
- [ ] T2.6 上下文供给·网页侧（FR-2.5）：仅选区 / ±相邻段落 / 附整页正文三档采集（tech §8.2）；素材卡片分栏展示；readability 失败降级显著标注；token 预览联动（FR-5）
...
- [ ] T3.6 “附 PDF 全文”档（FR-2.5）：viewer 全文供给 + 大文档截断策略（tech §8.2）
...
- [ ] T5.1 usage 精确模式：接入点按 FR-5.2 全量替换估算；估算仅兜底并带“≈”角标与口径 hover
...
- [ ] T5.5 估算校准（全局系数 + 一次账单反推；一并校准图片 token 公式）（tech §7.3）
